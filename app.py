...
def createfig(data, color_dropdown, xdata, ydata):
    ...
    fig.add_trace(
        go.Scatter(
            x=xdata, y=ydata, mode="lines", name="Signal", line_color=color_dropdown
        )
    )
    return fig
...
def animate(data, fig, points, speed=1):
    ...
    animation_settings = dict(
        frame=dict(duration=50 / speed, redraw=True), fromcurrent=True
    )

    fig.update_layout(
        title="Dynamic Signal Plot", xaxis_title="Time (s)", yaxis_title="Amplitude"
    )
    fig.update_layout(
        updatemenus=[
            dict(
                ...
                buttons=[
                        ...
                        args=[
                            {
                                "xaxis.range": None,
                                "yaxis.range": None,
                                "frame.duration": animation_settings["frame"][
                                    "duration"
                                ]
                                / 2,
                            }
                    ...
                    dict(
                        label="Hide Static Signal",
                        method="update",
                        args=[{"visible": [True, False]}],
                    ),
                    dict(
                        label="Show Static Signal",
                        method="update",
...
with col1:
    ...
    color = st.selectbox(
        "Change 1st signal Color", options=["Blue", "Red", "Green", "Yellow"]
    )
    ...
    if file is not None:
        ...
        st.write("The mean is :", mean, "The Standard Deviation is :", std)
        ...
        fig.add_trace(
            go.Scatter(
                x=x_static, y=y_static, mode="lines", name="Static Signal", visible=True
            )
        )
...
with col2:
    ...
    color1 = st.selectbox(
        "Change 2nd signal Color", options=["Blue", "Red", "Green", "Yellow"]
    )
    # Read data and plot dynamic graph
    if file1 is not None:
        ...
        st.write("The mean is :", mean1, "The Standard Deviation is :", std1)
